...
export async function fetchCommentsRequest({ clubId }: FetchCommentsPayload) {
  return await axios.get<CommentsResponse>(`${COMMENT_SERVER}/getComments/${clubId}`).then((res) => res.data);
}
...
export async function searchCommentsRequest({ clubId, keyword }: SearchCommentsPayload) {
  return await axios
    .get<CommentsResponse>(`${COMMENT_SERVER}/searchComment/${clubId}/${keyword}`)
    .then((res) => res.data);
}
...
export async function deleteCommentRequest({ commentId }: DeleteCommentPayload) {
  return await axios.delete<CommentResponse>(`${COMMENT_SERVER}/deleteComment/${commentId}`).then((res) => res.data);
}
...
export async function saveCommentRequest({ writer, clubId, content, responseTo }: SaveCommentPayload) {
  return await axios
    .post<CommentResponse>(`${COMMENT_SERVER}/saveComment`, {
      writer,
      clubId,
      content,
      responseTo,
