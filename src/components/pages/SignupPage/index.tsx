...
const SignupPage: FC<Props & RouteComponentProps> = ({ history, user }) => {
  ...
  async function onChangeId(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!/^[a-zA-Z0-9]{6,16}$/.test(e.target.value)) {
      ...
      const response = await Axios.post<DuplicateResponse>(`${USER_SERVER}/duplicateCheckId`, {
        id: e.target.value,
  ...
  async function onChangeNickname(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!/^[가-힣a-zA-Z0-9_ ]{2,8}$/.test(e.target.value)) {
      ...
      const response = await Axios.post<DuplicateResponse>(`${USER_SERVER}/duplicateCheckNickname`, {
        name: e.target.value,
      }).then((response) => response.data);
      if (response.success) {
        if (response.duplicate) {
  ...
  async function onChangeEmail(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!emailRegex.test(e.target.value)) {
      ...
      const response = await Axios.post<DuplicateResponse>(`${USER_SERVER}/duplicateCheckEmail`, {
  ...
  return (
    <Root>
      <Wrapper>
        ...
        <SignupForm description={text.loginId.description} value={id} onChange={onChangeId} />
        {idChecked && <VioletTextDiv>{idText}</VioletTextDiv>}
        {!idChecked && <RedTextDiv>{idText}</RedTextDiv>}
        <SignupForm description={text.nickname.description} value={name} onChange={onChangeNickname} />
        {nicknameChecked && <VioletTextDiv>{nicknameText}</VioletTextDiv>}
        {!nicknameChecked && <RedTextDiv>{nicknameText}</RedTextDiv>}
        <SignupForm description={text.email.description} value={email} onChange={onChangeEmail} type="email" />
